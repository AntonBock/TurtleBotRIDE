...
std::vector<int> iOrdVec;
    ...
    void _target_reached_cb(const actionlib::SimpleClientGoalState& state, const move_base_msgs::MoveBaseResultConstPtr& result)
    {
      ...
      srand (time(NULL));
        ...
        /* generate secret number between 0 and number of waypoints: */
...
/*
      //change for random
        for(int i=0; i < points.size()-1; i++){
            //std::swap(points[points.size()-2-i],points[points.size()-1-i]);
...
*/
    ...
    void _send_markers()
    {
        ...
        /*if (stops_initialized > 0)
        {
            ...
            marker.id = 0;
            marker.pose.position = points[0].point;
        ...
        {
            marker.header.frame_id = points[1].header.frame_id;
            //marker.header.frame_id = middle.header.frame_id;
            marker.id = 1;
            marker.pose.position = points[1].point;
            //marker.pose.position = middle.point;
            marker.pose.orientation.w = -0.7071;
            marker_array.markers.push_back(marker);
        }
        if (stops_initialized > 2)
        {
            marker.header.frame_id = points[2].header.frame_id;
            //marker.header.frame_id = from.header.frame_id;
            marker.id = 2;
            marker.pose.position = points[2].point;
    ...
    void _clicked_point_cb(const geometry_msgs::PointStamped::ConstPtr& msg)
    {
        ...
        {
          for(int i=0; i < points.size()-1; i++){
              //std::swap(points[points.size()-1-i],points[points.size()-2-i]);
              std::swap(points[i],points[i+1]);
            }
            points.back() = *msg;
        }
